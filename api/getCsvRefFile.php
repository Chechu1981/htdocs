...
$oldFile = $contacts->getLastRefFile($nplacas[trim($_POST['placa'])],$ref);
...
for($i = 0;$i < sizeof($oldFile);$i++){
  ...
  $htmlList .= '<ul '.$marcado.'>
    ...
    <li title="'.$oldFile[$i]["designacion"].'" class="info"><span></span>'.$contacts->formatRef($oldFile[$i]["referencia"]).$alerta.'</li>
...
echo "<h2>/TODO\ ".$placas[$oldFile[0]['placa']] . " - ".$oldFile[0]["libre"]." - ".sizeof($oldFile)." lineas ".$listReemp."</h2>" . $htmlList;
