<style>
  ...
  .list{
    list-style: none;
    display: grid;
    grid-template-columns: 8% 7% 13% 9% 10% 5% 6% 7% 16% 5% 4% 4%;
    align-items: center;
...
$hora = explode(" ",$_GET['date'])[1];
$htmlList = '<h2 style="text-align:center">Bajadas del '.$dia.'/'.$mes.'/'.$ano.' '.$hora.'</h2><ul class="list header">
<li>fecha</li>
<li>cuenta</li>
<li>Cliente</li>
<li>referencia</li>
...
$rows = $contacts->getRefDown(str_replace('-','/',$_GET['date']));
if(count($rows) > 0){
  foreach ($rows as $row) {
