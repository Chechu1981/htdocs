...
const disgon = (esDisgon) =>{
  ...
  if(esDisgon && destino == 'VIGO'){
    document.getElementsByClassName('form-group')[0].childNodes[15].appendChild(dsgLabel)
    document.getElementsByClassName('form-group')[0].childNodes[15].appendChild(dsgButton)
  }
  else if($('disgonBox')){
    $('disgonBox').remove()
    $('disgonLabel').remove()
  ...
  .then(response => {
    ...
    $('sendMail').addEventListener('click', (send) =>{
      ...
      for(let i = 1; i < $('cesiones').childNodes.length; i++){
        if($('cesiones').childNodes[i].localName == 'ul' && $('cesiones').childNodes[i].classList.value != "heading"){
          const id = $(`cesiones`).childNodes[i].childNodes[23].id
    ...
    for(let i = 1; i < $('cesiones').childNodes.length; i++){
      if($('cesiones').childNodes[i].localName == 'ul' && $('cesiones').childNodes[i].localName != undefined){
        //cesiones(origen,destino, $(`cesiones`).childNodes[i].childNodes[17].firstChild.checked)
        $(`cesiones`).childNodes[i].childNodes[17].firstChild.addEventListener('change', (e) => {
          ...
          const destino = e.target.parentNode.parentNode.childNodes[1].childNodes[3].innerHTML
          let cesion = null
        ...
        $(`cesiones`).childNodes[i].childNodes[21].firstChild.addEventListener('keyup', () => {
          updateChkbx()
        })

        const updateChkbx = () => {
          const data = new FormData()
          data.append('id', $(`cesiones`).childNodes[i].childNodes[23].id)
          ...
          data.append('tratado',$(`cesiones`).childNodes[i].childNodes[21].firstChild.value)
        ...
        $(`cesiones`).childNodes[i].childNodes[25].addEventListener('click', ()=>{
          const dataName = new FormData()
          dataName.append('id',$(`cesiones`).childNodes[i].childNodes[23].id)
          ...
          .then(enviado => 
            ...
            const referencia = $(`cesiones`).childNodes[i].childNodes[11].firstChild.data.replaceAll(' ','')
    ...
    for(let i = 1; i < $('cesiones').childNodes.length; i++){
      if($('cesiones').childNodes[i].localName == 'ul'){
        ...
        $('cesiones').childNodes[i].childNodes[5].addEventListener('click', (copy) =>{
          const copiar = `Cliente: ${$('cesiones').childNodes[i].childNodes[5].value}`
          var aux = document.createElement("input");
          aux.setAttribute("value", copiar);
          document.body.appendChild(aux);
          aux.select();
        ...
        $('cesiones').childNodes[i].childNodes[9].addEventListener('click', (copy) =>{
          const copiar = `${copy.target.childNodes[0].parentNode.textContent}`
          var aux = document.createElement("input");
          aux.setAttribute("value", copiar);
          document.body.appendChild(aux);
          aux.select();
...
document.addEventListener('click', (e) => {
  if(e.target.title.includes("Referencia: ")){
    ...
    navigator.clipboard.writeText(copiar.trim())
    notify(`${copiar} copiado!`)
  }
  const dataName = new FormData()
  dataName.append('id',e.target.parentNode.id)
  fetch('../api/isSend.php',{
    method: 'POST',
    body: dataName
  })
...
$$('form')[0].addEventListener('submit',(e)=>{
  ...
  data.append('session',window.location.href.split('=')[1])
  $('disgonBox') == null ? null : disgonStatus = true
