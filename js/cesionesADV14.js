...
$('origen').addEventListener('change',()=>{
  ...
  if($('origen').value != $('destino').value){
    ...
    $('newTitle').innerText = `${$('origen').value}>${$('destino').value}`
  }else{
    $('provider').innerText = ""
    $('pclient').innerText = ""
...
const showAssig = () =>{
  ...
  .then(response => {
    ...
    $('sendMail').addEventListener('click', (send) =>{
      ...
      for(let i = 1; i < $('cesiones').childNodes.length; i++){
        if($('cesiones').childNodes[i].localName == 'ul' && $('cesiones').childNodes[i].classList.value != "heading"){
          ...
          if(pedido == ''){
            alert('Faltan campos de pedido por completar')
            return false
          }
    ...
    for(let i = 0; i < $('cesiones').childNodes.length; i++){
      if($('cesiones').childNodes[i].localName == 'ul'){
        $(`cesiones`).childNodes[i].childNodes[15].firstChild.addEventListener('keyup', inp => {
          const data = new FormData()
          data.append('id', $(`cesiones`).childNodes[i].childNodes[21].id)
          data.append('pvp',$(`cesiones`).childNodes[i].childNodes[15].firstChild.value)
          data.append('pedido',$(`cesiones`).childNodes[i].childNodes[17].firstChild.value)
          data.append('value', $(`cesiones`).childNodes[i].childNodes[19].firstChild.checked)
        ...
        $(`cesiones`).childNodes[i].childNodes[17].firstChild.addEventListener('keyup', inp => {
          const data = new FormData()
          data.append('id', $(`cesiones`).childNodes[i].childNodes[21].id)
          data.append('pvp',$(`cesiones`).childNodes[i].childNodes[15].firstChild.value)
          data.append('pedido',$(`cesiones`).childNodes[i].childNodes[17].firstChild.value)
        ...
        $(`cesiones`).childNodes[i].childNodes[19].firstChild.addEventListener('change', inp => {
          const data = new FormData()
          data.append('id', $(`cesiones`).childNodes[i].childNodes[21].id)
          data.append('pvp',$(`cesiones`).childNodes[i].childNodes[15].firstChild.value)
          data.append('pedido',$(`cesiones`).childNodes[i].childNodes[17].firstChild.value)
        ...
        $(`cesiones`).childNodes[i].childNodes[23].addEventListener('click', ()=>{
          const pvp = $(`cesiones`).childNodes[i].childNodes[15].firstChild.value
          const pedido = $(`cesiones`).childNodes[i].childNodes[17].firstChild.value
          ...
          inout.append('origenF', origen+'F')
          if(pvp == '' || pedido == '')
            alert('Falta por rellenear los campos de pedido y precio')
          else if(confirm(`¿Enviar Correo?`)){
            const data = new FormData
            data.append('id',$(`cesiones`).childNodes[i].childNodes[21].id)
            data.append('value',$(`cesiones`).childNodes[i].childNodes[19].firstChild.checked)
            ...
            .then((item) => {
              ...
              .then(res => {
                ...
                createMail(cantidad,origen,destino,referencia,cliente,pedido,fragil,destinoFragil,res['origen'],res['destino'],res['conCopia'])
...
const createMail = (cantidad,origen,destino,referencia,cliente,pedido,fragil,destinoFragil,mailOrigen,mailDestino,bcc) =>{
  let mailFragil = ''
  let mailTarget = ''
  ...
  Saludos.`)
  Number.isInteger(parseInt(pedido)) ? mailTarget = mailBody : mailTarget = mailBodyNfm
  const data = new FormData();
...
$$('form')[0].addEventListener('submit',(e)=>{
  ...
  const pvp = $('pvp').value
  if(origen === destino){
    alert('El destino y el origen debe ser diferente')
    return false
  }
  if(cliente === ''){
    alert('Debes rellenar el cliente')
    return false
  }
  else if(ref === ''){
    ...
    alert('Debes rellenar la cantidad')
    return false
  }
  const data = new FormData()
  ...
  data.append('refClient',$('refClient').value)
...
botones.estadistica.addEventListener('click', (e) =>{
  ...
  .then((res) => {
    ...
    input.addEventListener("change",(e)=>{
      ...
      data.datasets.push({
        label: [res[e.target.value][0].toUpperCase()],
    ...
    data.datasets.push({
      label: [res[usuario][0].toUpperCase()],
      data: res[usuario][1],
      backgroundColor: colorArray,
      stack: 'Stack 0',
    })
