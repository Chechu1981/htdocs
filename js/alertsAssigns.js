const rutasDirectas = ["12864-1","105310-1","105251-1","11433-1"]
...
export const cesiones = (origen, destino,nfm,seg) =>{
  ...
  seg ? cesion += 'SEG' :''
  nfm ? cesion += 'NM' :''
  /* NO SE PUEDEN HACER CESIONES A GRANADA
  if(origen == 'GRANADA'){
    pclient.classList.add('important')
...
export const createInputExt = (placa) => {
  ...
  .then(proveedores => {
    ...
    pclient.querySelector('#refMat').addEventListener('change',(e) => {
      $('mailExt').innerHTML = e.target.value.split('~')[1]
    })
  })
}
